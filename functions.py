...
def apkopot_terinus(transakcijas):
    """
    Aprēķina tēriņu apkopojumu un atrod kategoriju ar lielākajiem tēriņiem.

    :param transakcijas: Saraksts ar transakcijām (category, amount, date).
    :return: Izvada kopējos tēriņus katrā kategorijā un dominējošo kategoriju.
    """
    kategoriju_izdevumi = defaultdict(float)

    # Saskaita tēriņus katrā kategorijā
    for kategorija, summa, _ in transakcijas:
        kategoriju_izdevumi[kategorija] += summa

    # Izvada kopējos tēriņus katrā kategorijā
    print("\nTēriņu apkopojums:")
    for kategorija, summa in kategoriju_izdevumi.items():
        print(f"{kategorija}: {summa:.2f} EUR")

    # Atrod kategoriju ar vislielākajiem tēriņiem
    if kategoriju_izdevumi:
        dominantā_kategorija = max(kategoriju_izdevumi, key=kategoriju_izdevumi.get)
        print(
            f"\nVisvairāk iztērēts kopā: {dominantā_kategorija} ({kategoriju_izdevumi[dominantā_kategorija]:.2f} EUR)")
    else:
        print("\nNav tēriņu šajā periodā.")
...
while True:
    ...
    if menu == "Y":
        ...
        cursor.execute(f"SELECT * FROM {user}_transactions ORDER BY date DESC LIMIT 3")
        last_three_entries = cursor.fetchall()

        if last_three_entries:
            print("\nLast 3 transactions:")
            for entry in last_three_entries:
                print(f"Category: {entry[0]}, Amount: {entry[1]:.2f} EUR, Date: {entry[2]}")
        ...
        if menu2 == "Y":
            ...
            if not df_transactions.empty:
                sns.barplot(data=df_transactions, x="category", y="amount", errorbar=None)
                plt.title(f" {user}'s expenses")
                plt.xlabel("Category")
                plt.ylabel("Amount (EUR)")
                plt.show()

                # Apkopot tēriņus un atrast lielāko tēriņu kategoriju
