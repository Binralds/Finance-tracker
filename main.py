...
class BudgetTrackerApp(App):
    ...
    def check_budget_compliance(self, df_summed):
        try:
            self.cursor.execute(f"SELECT total_budget FROM {self.user}_budgets WHERE category IS NULL")
            result = self.cursor.fetchone()
            if not result:
                return
            total_budget = result[0]

            df_budgets = pd.read_sql(f"SELECT category, budget FROM {self.user}_budgets WHERE category IS NOT NULL",
                                     self.connection)

            df_comparison = df_summed.merge(df_budgets, on="category", how="left")
            ...
            for _, row in df_comparison.iterrows():
                if row["amount"] > row["budget"] > 0:
                    warnings += f"- '{row['category']}' exceeded the budget by {row['amount'] - row['budget']:.2f} EUR\n"
                    over_budget = True

            total_spent = df_comparison["amount"].sum()
            if total_spent > total_budget:
                warnings += f"\n- Total spending exceeded the total budget by {total_spent - total_budget:.2f} EUR\n"
                over_budget = True

            if over_budget:
                self.show_popup("Budget Warnings", warnings)

        except Exception as e:
            print(f"Error in budget compliance check: {e}")
    ...
    def generate_report_charts(self, start_date, end_date):
        try:
            ...
            if not df_transactions.empty:
                ...
                self.check_budget_compliance(df_summed)
